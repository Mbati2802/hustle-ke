    ...
    <div className="space-y-5" onClick={() => setActionMenu(null)}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">{total} total · avg {avgRating}★ · {publicCount} public</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="bg-white rounded-xl border border-gray-200 p-3">
          <p className="text-xs font-semibold text-gray-500 mb-2">Rating Distribution</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 flex flex-col justify-between">
          <p className="text-xs font-semibold text-gray-500 mb-2">Average Rating</p>
          <div className="flex items-end gap-2">
            <p className="text-4xl font-bold text-amber-500">{avgRating}</p>
            <div className="flex items-center gap-0.5 mb-1.5">
              {[1,2,3,4,5].map(n => (
                <Star key={n} className={`w-4 h-4 ${n <= Math.round(Number(avgRating)) ? 'text-amber-400 fill-amber-400' : 'text-gray-200 fill-gray-200'}`} />
              ))}
            </div>
          </div>
            ...
            {['', 'true', 'false'].map(v => (
              <button key={v} onClick={() => { setVisibilityFilter(v); setPage(1) }}
                className={`px-2.5 py-1 text-[11px] rounded-lg font-medium transition ${
                  visibilityFilter === v ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                }`}>
                {v === '' ? 'All' : v === 'true' ? 'Public' : 'Hidden'}
              </button>
            ))}
          </div>
        </div>
